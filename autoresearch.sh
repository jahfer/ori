...
# Each run has a 30s timeout to prevent hangs from bugs
declare -a ops_results
declare -a total_results

for i in {1..5}; do
  output=$(timeout 30 ruby bench/ori_benchmark.rb 2>/dev/null)
...
last_output=$(timeout 30 ruby bench/ori_benchmark.rb 2>/dev/null)
